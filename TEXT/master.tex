...
%-------------- Main Content --------------
\section{Data:}
\subsection{Data Adjustment for Brazil:}

The series of top-income inequality for Brazil, from \cite{morgan2025distribution}, is based on a combination of tax data, national accounts and surveys, harmonized following the methodology of the World Inequality Database (WID). 
While this approach ensures macro-consistency, the authors highlight the presence of substantial volatility in the 1980s and early 1990s, a period characterized by extreme inflation prior to the \textit{Plano Real}.

This feature raises an important concern for the estimation of a Synthetic Control Method (SCM). 
Since SCM relies on a close fit of the pre-treatment trajectory, excessive volatility in the outcome variable may lead to an overfitting of transitory fluctuations rather than underlying trends, thereby biasing the estimation of the treatment effect. 
In particular, the sharp increase in top-income shares around the implementation of the \textit{Plano Real} may partly reflect measurement noise rather than a genuine structural change in inequality.

\par
The literature provides several mechanisms through which macroeconomic instability and inflation can distort tax-based measures of inequality. 
First, \cite{burkhauser_measuring_2015} show that taxable capital gains may be a poor proxy for actual income, particularly around periods of major economic or fiscal shocks. 
Second, and more directly relevant to this context, \cite{alvaredo2007richargentina} documents how hyperinflation in Argentina during the 1980s leads to substantial measurement issues in tax data, including increased tax evasion, underreporting of income, and incentives to postpone income declaration. 
These mechanisms imply that observed top-income shares during periods of very high inflation may be both noisy and biased, and therefore not fully comparable over time.

\par
In light of these considerations, I implement an adjustment procedure that treats inflation as a proxy for time-varying measurement noise. 
Rather than attempting to directly “correct” the level of inequality—which would require unavailable microdata—I adopt a shrinkage approach that reduces the influence of observations recorded during periods of extreme inflation.

\par
Specifically, I define a hyperinflation indicator based on the annual GDP deflator:

\begin{align}
hyper\_inflation_t =
\begin{cases} 
1 & \text{if } inflation_t > 200, \\ 
0 & \text{otherwise.}
\end{cases}
\end{align}

To assess the relationship between inflation and the volatility of top-income shares, I estimate the following panel regression across countries:

\begin{align}
top1\_inc_{it} = \gamma_0 + \gamma_1 hyper\_inflation_{it} + \gamma_2 hyper\_inflation_{it}^2 + \varepsilon_{it}
\end{align}

The results, reported in Table \ref{tab:regression}, indicate that periods of hyperinflation are associated with significant deviations in observed top-income shares, and that this relationship is non-linear. 
Importantly, this regression should not be interpreted as identifying a causal effect of inflation on inequality, but rather as capturing systematic patterns consistent with increased measurement noise during high-inflation episodes, but as a smoothing treatment to mitigate fluctuations partially attributable to measurement issues.
...
In particular, the study by \cite{morgan2025distribution} presents an extensive analysis of the evolution of income, minimum wages, and wealth in Brazil over a 72-year period (\textit{see, by combining data from various official Brazilian agencies and other non-official microdata sources}.
...
\begin{table}[H]
    ...
    \begin{tabular}{lcccc}
    \toprule
    \textbf{Dependent variable:} & \multicolumn{4}{c}{top1\_inc} \\
    \midrule
     & Coefficient & Std. Error & t-stat & p-value \\
    \midrule
    Constant                & 0.1382     & 0.001      & 128.135  & 0.000 \\
    hyper\_inflation        & 0.0002     & 4.13e-05   & 4.519    & 0.000 \\
    hyper\_inflation\_sq    & -5.606e-08 & 1.86e-08   & -3.016   & 0.003 \\
    \midrule
    Observations            & \multicolumn{4}{c}{1989} \\
    R-squared               & \multicolumn{4}{c}{0.020} \\
    Adjusted R-squared      & \multicolumn{4}{c}{0.019} \\
    F-statistic             & \multicolumn{4}{c}{20.65} \\
    Prob (F-statistic)      & \multicolumn{4}{c}{1.33e-09} \\
    Log-Likelihood          & \multicolumn{4}{c}{3218.4} \\
    AIC                     & \multicolumn{4}{c}{-6431} \\
    BIC                     & \multicolumn{4}{c}{-6414} \\
    \midrule
    Durbin-Watson           & \multicolumn{4}{c}{0.087} \\
    Omnibus                 & \multicolumn{4}{c}{105.563} \\
    Prob (Omnibus)          & \multicolumn{4}{c}{0.000} \\
    Jarque-Bera             & \multicolumn{4}{c}{118.913} \\
    Prob (JB)               & \multicolumn{4}{c}{1.51e-26} \\
    Skew                    & \multicolumn{4}{c}{0.586} \\
    Kurtosis                & \multicolumn{4}{c}{2.751} \\
    Cond. No.               & \multicolumn{4}{c}{2.02e+05} \\
    \bottomrule
    \end{tabular}
\end{table}
...
\begin{figure}
    \centering
    \includegraphics[width=1\textwidth]{brazil_inflation_x_topinc.png}
    \caption{Adjustment of top-income inequality series for Brazil during hyperinflation.}
    \label{fig:adjustment}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=1\textwidth]{us_ascm_mundi.png}
    \caption{Weights ASCM for the US}
    \label{fig:mundi_ascm}
\end{figure}
\end{document}